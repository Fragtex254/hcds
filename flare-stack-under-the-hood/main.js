...
(function () {
  ...
  function initDnD(containerEl) {
    ...
    // Track which answers are placed in each zone
    if (!containerEl._placedMap) containerEl._placedMap = {};
    ...
    function handleDrop(zoneTarget, answer, chip) {
      if (!zoneTarget || !chip) return;
      const container = zoneTarget.closest('.dnd-container');
      if (!container) return;
      const map = container._placedMap;

      // Remove from previous zone if re-dropping
      Object.keys(map).forEach(zId => {
        map[zId] = (map[zId] || []).filter(a => a !== answer);
        if (map[zId].length === 0) delete map[zId];
      });

      const zone = zoneTarget.closest('.dnd-zone');
      const zoneId = zone.dataset.zoneId || Array.from($$('.dnd-zone', container)).indexOf(zone).toString();
      if (!map[zoneId]) map[zoneId] = [];
      if (!map[zoneId].includes(answer)) map[zoneId].push(answer);

      chip.classList.add('placed');
      chip.classList.remove('dragging');

      // Re-render all zone targets
      renderPlacedChips(container);
    }

    function renderPlacedChips(container) {
      const map = container._placedMap;
      const allZones = $$('.dnd-zone', container);
      allZones.forEach((zone, idx) => {
        const target = $('.dnd-zone-target', zone);
        if (!target) return;
        const placed = map[idx.toString()] || [];
        target.classList.remove('correct-placed', 'incorrect-placed');
        if (placed.length === 0) {
          target.innerHTML = '<span style="color:var(--color-text-muted);font-size:var(--text-sm)">拖到这里</span>';
        } else {
          target.innerHTML = '';
          placed.forEach(answer => {
            const chip = $(`.dnd-chip[data-answer="${answer}"]`, container);
            if (!chip) return;
            const tag = document.createElement('span');
            tag.className = 'dnd-placed-tag';
            tag.textContent = chip.textContent;
            tag.dataset.answer = answer;
            tag.style.cssText = 'display:inline-block;padding:var(--space-1) var(--space-3);background:var(--color-accent-light);color:var(--color-accent);border-radius:var(--radius-full);font-size:var(--text-sm);font-weight:600;margin:var(--space-1);cursor:pointer;';
            tag.title = '点击移除';
            tag.addEventListener('click', function(e) {
              e.stopPropagation();
              const chipEl = $(`.dnd-chip[data-answer="${answer}"]`, container);
              if (chipEl) chipEl.classList.remove('placed');
              const zoneMap = container._placedMap;
              zoneMap[idx.toString()] = (zoneMap[idx.toString()] || []).filter(a => a !== answer);
              if (zoneMap[idx.toString()].length === 0) delete zoneMap[idx.toString()];
              renderPlacedChips(container);
            });
    ...
    chips.forEach(chip => {
      ...
      chip.addEventListener('touchend', e => {
        ...
        const el    = document.elementFromPoint(touch.clientX, touch.clientY);
        const zt    = el && el.closest('.dnd-zone-target');
        if (zt) handleDrop(zt, chip._answer, chip);
        zones.forEach(z => { const t = $('.dnd-zone-target', z); if (t) t.classList.remove('drag-over'); });
      });
    });
  }
  ...
  window.checkDnD = function (containerId) {
    ...
    allZones.forEach((zone, idx) => {
      ...
      const correctSet = new Set(
        (zone.dataset.correct || '').split(',').map(s => s.trim()).filter(Boolean)
      );

      // Only mark correct if ALL placed chips are correct and count matches
      const allCorrect = placed.every(a => correctSet.has(a)) && placed.length === correctSet.size;
      target.classList.add(allCorrect ? 'correct-placed' : 'incorrect-placed');
    });
  };
  ...
  window.resetDnD = function (containerId) {
    const container = $('#' + containerId);
    if (!container) return;
    container._placedMap = {};
    $$('.dnd-zone-target', container).forEach(t => {
      t.innerHTML = '<span style="color:var(--color-text-muted);font-size:var(--text-sm)">拖到这里</span>';
